...
- **Team server mode is now connect-to-existing only.** Removed the misleading "1. Connect to an existing PostgreSQL database / 2. Connect to a new database (I'll create it)" sub-choice from `promptTeamConnection()`. The "I'll create it" option (2) was a no-op lie — `bootstrapDatabase()` only probes and runs migrations; nothing was ever created. The team flow is now: host, port (default 6200), database, username (default `neuralgentics`), password.
- **Port default changed from 5432 to 6200** in the team-connection prompt and the `--yes --team` defaults, matching the canonical port used by `compose.example.env` and the repo docs.
- **DSN password encoding fix.** User and password are now URL-encoded via `encodeURIComponent` when building PostgreSQL DSNs in `db-setup.ts`, `prompts.ts`, and `init.ts`. Passwords containing `@`, `:`, `/`, or other reserved characters previously broke the DSN.
- **Better failure message when the team probe fails.** `bootstrapDatabase()` now prints actionable guidance pointing the user at `npx @veedubin/neuralgentics --db-start` instead of a bare "Cannot connect..." message.

### Added

- **`--db-start` / `--db-stop` CLI commands** — ship a first-class compose file + database helper. `--db-start` writes the shipped `docker-compose.yml` + `compose.example.env` to `~/.neuralgentics/` (backing up any existing compose file with a timestamp — never destroys user edits), detects `podman-compose` / `podman compose` / `docker compose` in that preference order, runs `up -d`, waits for `pg_isready` (30s max), offers to create the user's first database user (see "First-user bootstrap" below), and prints the DSN to paste into `--init-project`. `--db-stop` runs `down` (NOT `down -v` — volumes are never deleted; the Container Deletion Policy applies to our own tooling).
- **Named, multi-instance compose.** The shipped `docker-compose.yml` service is now named `db-server` (a generic, role-based name) instead of `neuralgentics-postgres`. Every resource is parameterised by `${NEURALGENTICS_STACK_NAME:-neuralgentics}`: container name (`${STACK}-db`), volume name (`${STACK}-db`, via the volume's `name:` property), network name (`${STACK}-net`). Copy the env file, set a different `NEURALGENTICS_STACK_NAME` AND `NEURALGENTICS_DB_PORT`, and you can run a second stack alongside the first (e.g. one per project) — both stacks share the same compose file, only the env file differs.
- **First-user bootstrap in `--db-start`.** After the stack is up and ready, `--db-start` offers to create the user's first database user instead of leaving them on the default `neuralgentics`/`neuralgentics` superuser. Interactive: `Create your first database user now? [Y/n]` → username (default `$USER`) + password → `CREATE USER` + `GRANT ALL PRIVILEGES ON DATABASE` via `podman exec` psql inside the `${STACK}-db` container using the `POSTGRES_USER`/`POSTGRES_DB` admin identity. "User already exists" (PostgreSQL role error) is treated as success. Non-interactive flags: `--db-user <name>` + `--db-password <pw>` (and `--yes` to skip the offer entirely). If the user declines, the exact `podman exec` psql one-liner is printed for later use, with a note that the default superuser works but shouldn't be shared. The final DSN (`postgresql://<user>:<url-encoded-pw>@localhost:<port>/<db>`) is printed for the created user.
  - SQL injection / quoting: usernames are validated against `^[a-zA-Z_][a-zA-Z0-9_]*$` (reject anything with hyphens, digits-first, quotes, semicolons, etc.) and used as double-quoted identifiers; passwords are single-quote-escaped (`'` → `''`) in the SQL literal and URL-encoded in the printed DSN.
- **Shipped `docker-compose.yml` + `compose.example.env` inside the npm package.** The compose file ships a single rock-solid PostgreSQL service (image `ghcr.io/veedubin/neuralgentics-postgres:latest`, service name `db-server`, container `${NEURALGENTICS_STACK_NAME:-neuralgentics}-db`, named volume `${STACK}-db` via the `name:` property, port `${NEURALGENTICS_DB_PORT:-6200}`, `pg_isready` healthcheck, `restart: unless-stopped`). The sidecar and backend services are commented out — uncomment when you need them. Both files are added to the package `files` field so they land in the tarball.
- **`db-stack.ts` module** (new) — runtime detection (`detectComposeRuntime()`), no-overwrite backup behaviour (`ensureStackFiles()`), env-file parsing (`parseEnvFile`), stack-config resolution (`resolveStackConfig` honours `.env` + `process.env`), SQL password escaping (`escapeSqlPassword`), first-user creation (`createFirstUser`), DSN building (`buildUserDSN`, URL-encodes password), and `db-stop` never passes `-v`. Unit tests added (`db-stack.test.ts`) covering runtime detection order, no-overwrite backup behaviour, DSN printing, env-file parsing, stack-config resolution, SQL password escaping, username regex rejection (bad names like `1bad`, `weird-name`, `'; DROP TABLE users; --`), first-user non-interactive path, "user already exists" treated as success, declined-offer path prints the later-command, `--yes` skips the offer, and stack-name interpolation into the exec target + DSN (multi-instance). 43 tests pass.

### References

- `prompts.ts`, `db-setup.ts`, `init.ts`, `cli.ts`, `db-stack.ts` (new), `db-stack.test.ts` (new), `docker-compose.yml` (new, shipped — service named `db-server`, multi-instance via `NEURALGENTICS_STACK_NAME`), `compose.example.env` (new, shipped — documents `NEURALGENTICS_STACK_NAME` + multi-instance recipe), `package.json` `files` field, overlay README (container stack + multi-instance + first-user bootstrap sections), shipped AGENTS.md (container stack + why-create-your-own-user + multi-instance sections).
