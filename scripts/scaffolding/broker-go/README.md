...
[![CI](https://github.com/Veedubin/neuralgentics-broker/actions/workflows/ci.yml/badge.svg)](https://github.com/Veedubin/neuralgentics-broker/actions/workflows/ci.yml)
[![Go](https://img.shields.io/badge/Go-1.25-%2300ADD8)](https://go.dev/)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)

A Go **MCP (Model Context Protocol) broker** with built-in tool-call audit,
hot-reload, and an optional transport swap to route outbound HTTP through a
centralized egress proxy. Part of the [neuralgentics](https://github.com/Veedubin/neuralgentics)
...
- Proxies JSON-RPC calls between a client (e.g. an LLM agent) and the servers
...
- Optionally routes the servers' outbound HTTP through the
  ...
  egress proxy (the **chokepoint / policy proxy** of the ecosystem)
- Supports hot-reload of the server config without dropping in-flight
  connections

### Broker vs. Gateway

The two are easy to confuse — the split is:

| Component | Role | Repo |
|-----------|------|------|
| **neuralgentics-broker** | **Lifecycle manager**: spawns, supervises, and audits MCP server subprocesses. Owns the tool-call audit log. | this repo |
| **neuralgentics-gateway** | **Chokepoint / egress proxy**: enforces outbound HTTP policy, rate-limits, and audits HTTP traffic leaving the network. | [Veedubin/neuralgentics-gateway](https://github.com/Veedubin/neuralgentics-gateway) |

You can run the broker without the gateway (audit-only, no HTTP policy). You
can run the gateway without the broker (proxy-only, no MCP lifecycle). When
both run together, the broker's servers route their outbound HTTP *through*
the gateway — giving you a single policy enforcement point for all
MCP-driven network egress.

### Ports

The broker itself does not listen on a TCP port — it exposes an MCP server
on **stdio** for the agent to connect to. Set `--rpc-timeout` to bound each
JSON-RPC call. Outbound HTTP from spawned servers goes through the gateway
port (default `9090`) when `EGRESS_GATEWAY_URL` is set.

## What this is NOT

- Not a memory store. (Use [memini-ai](https://github.com/Veedubin/memini-ai-dev)
  for that.)
- Not a proxy / egress gateway. (Use
  [neuralgentics-gateway](https://github.com/Veedubin/neuralgentics-gateway)
  for that.)
- Not a web UI. (Use
  [neuralgentics-web](https://github.com/Veedubin/neuralgentics-web) for
  that.)
...
```bash
git clone https://github.com/Veedubin/neuralgentics-broker.git
cd neuralgentics-broker
go build ./...
```
...
```bash
go test ./...
# with race detector + fresh test run (as CI does):
...
The [`neuralgentics-web`](https://github.com/Veedubin/neuralgentics-web)
`broker-audit` module can read this table and render it as a live dashboard.
...
- [neuralgentics-web](https://github.com/Veedubin/neuralgentics-web) — web UI that can visualize broker audit logs
