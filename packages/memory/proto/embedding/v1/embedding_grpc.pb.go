...
type EmbeddingServiceClient interface {
	...
	EmbedBatch(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EmbedRequest, EmbedResponse], error)
...
func (c *embeddingServiceClient) EmbedBatch(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[EmbedRequest, EmbedResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &EmbeddingService_ServiceDesc.Streams[0], EmbeddingService_EmbedBatch_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type EmbeddingService_EmbedBatchClient = grpc.BidiStreamingClient[EmbedRequest, EmbedResponse]
...
type EmbeddingServiceServer interface {
	...
	// EmbedBatch generates embeddings for multiple texts in a streaming fashion.
	EmbedBatch(grpc.BidiStreamingServer[EmbedRequest, EmbedResponse]) error
	// Health checks if the sidecar is ready to serve requests.
...
func (UnimplementedEmbeddingServiceServer) EmbedBatch(grpc.BidiStreamingServer[EmbedRequest, EmbedResponse]) error {
	return status.Error(codes.Unimplemented, "method EmbedBatch not implemented")
}
...
func _EmbeddingService_EmbedBatch_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EmbeddingServiceServer).EmbedBatch(&grpc.GenericServerStream[EmbedRequest, EmbedResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EmbeddingService_EmbedBatchServer = grpc.BidiStreamingServer[EmbedRequest, EmbedResponse]
