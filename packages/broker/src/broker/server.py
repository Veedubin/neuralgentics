...
from broker.models import ServerConfig, ToolCall, ToolSummary, DeregisterRequest
...
def call_tool(body: ToolCall) -> dict:
    ...
    try:
        result = proxy.invoke(
            server_name=body.server, method=body.tool, params=body.arguments
        )
    except Exception as exc:
...
def deregister_server(body: DeregisterRequest) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Server '{body.name}' not found"
        ) from exc
