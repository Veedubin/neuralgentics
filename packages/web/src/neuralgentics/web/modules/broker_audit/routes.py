...
from neuralgentics.web.modules.broker_audit.sse import BrokerAuditBroadcaster, Goodbye
...
def build_router(
    ...
    *,
    ...
    @router.get("/modules/broker-audit/charts", response_class=HTMLResponse)
    @module_route("view_audit")
    async def broker_audit_charts(
        request: Request,
        user: User | None = Depends(_dep("view_audit", ("admin", "operator", "viewer"))),
    ) -> HTMLResponse:
    ...
    async def sse_stream(
        ...
        _ = user  # noqa: F841 — RBAC gate only; read endpoint
        q: asyncio.Queue[BrokerAuditEvent | Goodbye] = broadcaster.subscribe()
            ...
            try:
                while True:
                    if await request.is_disconnected():
                        break
                    try:
                        event: BrokerAuditEvent | Goodbye = await asyncio.wait_for(
                            q.get(), timeout=5.0
                        )
