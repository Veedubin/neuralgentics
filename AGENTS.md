...
- **v0.16.4** (2026-07-30) is the latest tagged release. CI-only: makes `publish-npm` job idempotent on tag re-push — adds `npm view @veedubin/neuralgentics@v$VERSION` pre-check; skips publish with `::notice::` if version already on npm. Triggered because v0.16.3 workflow re-run (after tag rebase) failed with `npm error You cannot publish over the previously published versions: 0.16.3`. v0.16.3 release itself shipped correctly (npm + GH Release + 3 container images all published); the failure was CI noise from a redundant workflow trigger. **Card T-NPM-REPUB-GUARD-001.** Future tag re-pushes (e.g., after another rebase) will be clean green runs.
- **v0.16.3** (2026-07-30) — 3 high-UX-impact fixes: T-MEMADD-001 (neuralgentics_memory_add schema now exposes 'content', tolerant of legacy 'text'); T-LOG-RED-001 (Go backend stderr no longer rendered as red TUI errors — INFO routed to plugin logger, WARN+ to TUI with [neuralgentics:backend] prefix); T-INIT-TIMEOUT-001 (--init-project and --init-homedir now propagate entry.timeout, shared applyTemplateEntry() helper extracted, template-parity tests added). validate-release.sh 29 pass / 0 fail. Also closes pre-existing version-drift on root package.json + packages/tui/package.json + install.sh DEFAULT_VERSION (now all 0.16.3 alongside overlay's 0.16.3). **Workflow note:** a tag re-push after rebase triggered a second Release run that correctly failed the npm publish step (npm refuses to republish same version). v0.16.4 fixes this by guarding the publish-npm job.
