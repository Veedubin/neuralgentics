---
description: Neuralgentics Reviewer - Code quality and security review using deepseek-v4-pro:cloud (Ollama Cloud) with memini-core for context.
mode: subagent
...
2. **Query memoryManager FIRST** — `memoryManager_query_memories` for previous review decisions and patterns
3. **Use thought chains** — `memoryManager_add_thought` for complex review analysis
4. **Save when complete** — `memoryManager_add_memory` with review findings
