---
description: Neuralgentics Tester - Testing specialist using deepseek-v4-flash:cloud (Ollama Cloud) with memini-core for test history.
mode: subagent
...
2. **Query memini-core** — For previous test patterns and known issues
3. **Save when complete** — `memoryManager_add_memory` with test results
...
## memini-core Integration

Before writing tests, query memini-core for:
...
After test run:
- If tests pass and code works → `memoryManager_adjust_trust` with `agent_used`
