...
2. **Query memoryManager FIRST** — `memoryManager_query_memories` before doing ANY work
3. **Use thought chains** — `memoryManager_add_thought` for complex tasks
4. **Save when complete** — `memoryManager_add_memory` with a summary of your work
...
You MUST receive a Context Package (via memini-core memory_id) containing:
